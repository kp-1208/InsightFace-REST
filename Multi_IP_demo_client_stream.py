...
class IFRClient:
    ...
    def detect_from_live_stream(self, frames_buffer, mode='data', threshold=0.6, extract_embedding=True, return_face_data=True,
                                return_landmarks=True, embed_only=False, limit_faces=0, use_msgpack=True):
            
        while True:
            if frames_buffer.qsize() < BATCH_SIZE:
                time.sleep(0.1)
    ...
    def extract(self, data: list,
        ...
        print(f"Batch of {len(images)} frames processed in: {end-start}")
        #images.clear()
...
def main(shared_buffer):
    ...
    client.detect_from_live_stream(shared_buffer, mode='data', threshold=0.6, extract_embedding=to_bool(args.embed),
